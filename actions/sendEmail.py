...
class Mails(Action):
    def run(self, incident, priority, summary, notes, validationResults,
                    ...
                    userName,pwd,primarySmtpAddress):
        ...
        try:
            if types == "nual":
                ...
                content_body = "https://itsm.lenovo.com/sso/mailclick.asp?u=/servlet/ViewFormServlet?form=Lenovo_INC_OpenTicketByURL&server=lenovoargroup&F1000000161=%s" % incident + "\n" +\
                           "Priority: %s" % priority + "\n" + "Summary: %s" % summary + "\n" + "Notes: %s" % notes + "\n" + \
                ...
                content_body = "******************** Auto Quick Fix Link/自修复工具地址 ********************" + "</br>" * 3 + "<a href=http://ccaop.lenovo.com/#/autofixAWP?source=ITSM&ips=%s&roleid=%s&verifyCode=%s&autoQuickType=>AutoQuickFix:Please click here to do the QuickFix</a>" % (
                ip, roleId, action) + "</br>" + "incident:%s" % incident + "</br>" + "priority:%s" % priority + "</br>" + "summary:%s" % summary
